...
def main():
    ...
    SPECS = {
        ...
        "RegimeGate_oracle": (
            StrategyBlockBandit,
            CJ,
            {"policy": "uniform", **WARM_ON, "regime_gate": "oracle"},
            ARCHIVE,
        ),
    ...
    setup = (
        f"from {src}"
        if src
        else f"{battery.name}, budget {battery.budget_multiplier}*d, insts {list(battery.instances)}"
    )
    if fids_seen:
    ...
    if best_warm and best_ref:
        gates.append(
            ("G5", f"best warm ({best_warm}) - best single ({best_ref})", delta(best_warm, best_ref), 0.0, ">=")
        )
    if not gates:
