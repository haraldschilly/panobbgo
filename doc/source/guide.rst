...
.. list-table::
   ...
   * - **Want to measure progress?**
     - Read :doc:`guide_benchmarking` for the composite score, external baselines, parametrically randomised problems, the statistical acceptance rule, the autonomous self-improvement loop driver, the anti-cherry-pick guard, the adaptive Thompson-sampling mutation sampler, the structural ``add_heuristic`` / ``drop_heuristic`` portfolio mutations, and the dual-topology PSO (``gbest`` / ``lbest``) candidate pool
