...
- **NLSHADE_RSP**: NL-SHADE-RSP (Stanovov, Akhmedova & Semenkin, CEC 2020 winner) — direct subclass of :class:`~panobbgo.heuristics.jso.JSO` that adds two refinements on top of jSO: **rank-based selective pressure (RSP)** — the differential donor ``r1`` is drawn with probability that decays exponentially with fitness rank (``w_i = exp(−kp·i/N)``, ``kp = 3`` by default), so fitter individuals are picked more often and the ``F·(x_r1 − x_r2)`` term acquires a mild toward-the-good-basin drift; and an **adaptive archive size** — the external-archive cap is re-drawn each generation from ``randint(0, A_max + 1)`` (default enlarged ``archive_factor = 2.6``) so the diversity of the ``r2`` donor pool varies generation-to-generation.  ``kp → 0`` recovers jSO's uniform donor selection exactly, so the selective pressure is a continuous dial the loop can tune.  Inherits jSO's weighted ``current-to-pbest-w/1`` mutation, linear ``p_best`` schedule, asymmetric F-cap, frozen-anchor memory bin, and the whole asynchronous pipeline unchanged.  Opt-in via the structural mutation catalog.
- **DifferentialEvolution / LSHADE / JSO / NLSHADE_RSP complementarity**: Panobbgo carries all four DE-family arms in the structural catalog so the bandit can pick whichever wins on the current battery — basic DE for byte-identical legacy reproduction, L-SHADE for the established CEC-2014 high-water mark, jSO for the CEC-2017 refinement, and NL-SHADE-RSP for the CEC-2020 rank-based-selective-pressure refinement.  Each variant occupies a distinct exploration / exploitation regime and the literature shows real per-problem complementarity.
