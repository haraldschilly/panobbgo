# Self-improvement loop — flag and feature reference

**Status: the loop is not currently in use.** The nightly workflow
(`.github/workflows/self_improve_nightly.yml`) has been disabled on GitHub
since 2026-08-13 after a 34-night audit (`planning/done/LOOP_DIAGNOSIS_2026-08-11.md`)
found that its accept rule sits on the harness's measurement-noise floor.
The driver (`panobbgo/self_improve.py`, `scripts/self_improve.py`) and its
tests remain in the tree and still run. This file is the reference for its
CLI flags, ledger records and internal mechanisms — moved verbatim (in
reStructuredText syntax) from the "Self-improvement loop" section of
`doc/source/guide_benchmarking.rst` during the 2026-09-09 documentation
consolidation so the user-facing benchmarking guide stays short. Design:
`planning/SELF_IMPROVEMENT_LOOP.md`; dated history:
`planning/SELF_IMPROVEMENT_LOG.md`; the AGENTS.md changelog that
accompanied these features: `planning/done/AGENTS_loop_changelog_2026.md`.
