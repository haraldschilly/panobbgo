...
def test_on_new_results_no_best():
    ...
    strategy.best = None
    analyzer.on_new_results([Result(Point(np.array([0.]), "test"), 1.0)])
...
def test_on_new_results_best_fx_none():
    ...
    strategy.best = Result(Point(np.array([0.]), "test"), None)
    analyzer.on_new_results([Result(Point(np.array([0.]), "test"), 1.0)])
...
def test_check_convergence_cv_improving():
    ...
    for cv in cv_values:
        strategy.best = Result(Point(np.array([0.]), "test"), 1.0, cv_vec=np.array([cv]))
        strategy.results.append(strategy.best)
        analyzer.on_new_results([strategy.best])
